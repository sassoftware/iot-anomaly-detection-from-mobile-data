...
function M.mqtt_subCallback(topic,payload)
    ...
    if not decoded then
        print( "M.mqtt_subCallback:  Decode failed at "..tostring(pos)..": "..tostring(msg) )
    else
        ...
        if table2["deviceID"] == Glb.deviceID then 
            ...
            if  table2.min_dist > .8 then 
                M.Indicator:begin_clicked()
                M.title.text = "Danger" 
            elseif  table2.min_dist > .3 then 
                M.Indicator:begin_rollover()
                M.title.text = "Warning" 
            elseif  table2.min_dist < .2 then 
...
local create_new = function ( self, event )
	...
	M.sceneGroup.UI_inst = UI:newUI( { title="Template", stage = M.sceneGroup , navitem = 4 } )
	------------------------------
	-- CONFIGURE STAGE
	------------------------------
